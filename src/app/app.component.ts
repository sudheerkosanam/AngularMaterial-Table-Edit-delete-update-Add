...
export class AppComponent implements OnInit {
  ...
  startEdit(i: number, id: number, firstname: string, lastname: string, email: string, phone: string) {
    ...
    const dialogRef = this.dialog.open(EditDialogComponent, {
      data: {id: id, firstname: firstname, lastname: lastname, email: email, phone: phone}
    });
  ...
  connect(): Observable<Issue[]> {
    ...
    return merge(...displayDataChanges).pipe(map( () => {
        ...
        this.filteredData = this._exampleDatabase.data.slice().filter((issue: Issue) => {
          const searchStr = (issue.id + issue.firstname + issue.email + issue.phone).toLowerCase();
  ...
  sortData(data: Issue[]): Issue[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'created_at': [propertyA, propertyB] = [a.phone, b.phone]; break;
